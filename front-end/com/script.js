...
for (let i = 0; i < column; i++) {
  columnsRendered++;
  columnIns += `<div class="column" id="column${columnsRendered}">${rowInsRender(
    i
  )}</div>`;
}
  ...
  for (let i = 0; i < block; i++) {
    ...
    if (con === undefined) {
      if (blocksRendered % 100 === 0) {
        blockIns += `<div class="block arrived" id="arrived${
          blocksRendered / 100
        }"> </div>`;
      } else {
      ...
      if (blocksRendered % 100 === 0) {
        blockIns += `<div class="block ${con.extra[i]} arrived" id="arrived${
          blocksRendered / 100
        }">${con.text[i] === undefined ? " " : con.text[i]}</div>`;
      } else {
        blockIns += `<div class="block ${con.extra[i]}">${
          con.text[i] === undefined ? " " : con.text[i]
        }</div>`;
      }
    }
  }
